...
async def get_email_code(conn: AsyncSession, email: str, code_type: str) -> tp.Optional[email_models.EmailSend]:
    try:
        email_code = (await conn.exec(
            select(email_models.EmailSend).
            where(email_models.EmailSend.email == email, email_models.EmailSend.type == code_type),
        )).scalars().one()
...
async def send_code(
    ...
    code = generate_code()
    while (await conn.exec(
        select(email_models.EmailSend).
        where(email_models.EmailSend.code == code, email_models.EmailSend.expired_at < datetime.datetime.now()),
    )).fetchall():
        code = generate_code()
