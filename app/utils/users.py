...
async def get_current_user(request: Request, conn: tp.Annotated[AsyncSession, Depends(db.get_db)]):
    ...
    if (fingerprint is None) or (not await authenticate_user_session(conn=conn, fingerprint=fingerprint, user_id=user_id)):
        raise credentials_exception
...
async def get_session(user_id: int, fingerprint: str, conn: AsyncSession):
    query = select(users.Sessions.user_id, users.Sessions.session_id).where(users.Sessions.user_id == user_id, users.Sessions.session_id == fingerprint)
    return (await conn.exec(query)).fetchone()
